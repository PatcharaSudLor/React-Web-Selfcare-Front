    ...
    return (
      <div className="flex flex-col h-auto bg-gray-50">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
              <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm ${msg.role === 'user' ? 'bg-blue-500' : 'bg-green-500'
                  }`}>
        ...
        <div className="p-4 bg-white border-t border-gray-100">
          ...
          {!isLoading && (
            <div className="w-full flex gap-2 mb-3 flex-wrap">
              <button
                ...
                className="text-xs px-3 py-1.5 bg-green-50 text-green-700 border border-green-200 rounded-full hover:bg-green-100 transition-colors flex items-center gap-1"
              >
                💪 สร้างตารางออกกำลังกาย
              </button>
              <button
                onClick={() => handleSend('ช่วยวางแผนอาหาร 7 วันหน่อย')}
                className="text-xs px-3 py-1.5 bg-orange-50 text-orange-700 border border-orange-200 rounded-full hover:bg-orange-100 transition-colors flex items-center gap-1"
              >
                🍽️ สร้างแผนอาหาร
              </button>
              <button
          ...
          <div className="w-full flex gap-2 bg-gray-100 p-1.5 rounded-full items-center px-4 focus-within:ring-2 focus-within:ring-green-400 transition-all">
            <input
            ...
            <button
              onClick={() => handleSend()}
              disabled={isLoading}
              className={`p-2 rounded-full transition-all ${!input.trim() || isLoading ? 'bg-gray-300' : 'bg-green-500 hover:bg-green-600'
                }`}
            >
